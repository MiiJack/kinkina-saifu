...
public class BalanceService {
    ...
    public double getBalanceSummary(int accountId, LocalDateTime date, BalanceSummaryType type) throws SQLException {
        ...
        List<TransferHistory> transferHistoryList = transferHistoryCrudOperations.findAllWithinRange(
        ...
        if (latestTransaction != null) {
            ...
            for (TransferHistory transferHistory : transferHistoryList) {
                ...
                Transaction sourceTransaction = transactionService.findLatestTransaction(transferHistory.getDebtorTransferId(), date);
                Transaction targetTransaction = transactionService.findLatestTransaction(transferHistory.getCreditorTransferId(), date);
                ...
                double conversionRate = otherCrudOperations.getCurrencyValue(sourceCurrency.getId(), targetCurrency.getId());
        ...
        switch (type) {
            ...
            case MEDIAN:
                ...
                int medianIndex = (amounts.size() - 1) / 2; // Calculate the median index (middle of the sorted list)
